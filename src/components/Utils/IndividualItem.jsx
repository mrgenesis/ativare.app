...
export default function IndividualItem({ response, type }) {
  const [state] = React.useContext(Context);
  const classes = useStyles();
  const [displayEdit, setDisplayEdit] = React.useState('display');
  const [item, setItem] = React.useState(response);
  const edit = () => setDisplayEdit('edit');
  const display = () => setDisplayEdit('display');
  const componentsEdit = {
    material: <EditMaterial item={item} setItem={setItem} />,
    product: <EditProduct item={item} setItem={setItem} />
  }

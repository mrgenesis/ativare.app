...
module.exports = function setContext(context) { 
  ...
  async function receiveAadData(reqHandler, reqBody) {    
    ...
    if (types.areDifferents(state.csrfToken, reqHandler.getCsrfTokenSession())) {
    ...
    try {
      tokenResponse = await msalInstance.acquireTokenByCode(reqHandler.authCodeRequest);
    ...
    return `${context.appConfig.dataHost.frontendBaseUrl}${state.redirectTo}`;    
  }
